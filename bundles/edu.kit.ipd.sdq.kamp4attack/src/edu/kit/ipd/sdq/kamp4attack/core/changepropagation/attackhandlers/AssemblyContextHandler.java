...
public abstract class AssemblyContextHandler extends AttackHandler {
	...
	private void handleDataExtraction(final Collection<CompromisedAssembly> components) {
		...
		final var dataList= new LinkedList<DatamodelAttacker>();
		for (AssemblyContextDetail filteredComponent : filteredComponents) {
			dataList.addAll(filteredComponent.getAssemblyList().stream().distinct()
	...
	protected abstract Optional<CompromisedAssembly> attackComponent(AssemblyContextDetail component, CredentialChange change,
			EObject source);
