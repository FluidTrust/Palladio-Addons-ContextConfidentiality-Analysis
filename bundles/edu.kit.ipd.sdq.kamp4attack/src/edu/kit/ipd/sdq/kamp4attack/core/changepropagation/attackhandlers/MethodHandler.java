...
public abstract class MethodHandler extends AttackHandler {
	...
	private void handleDataExtraction(final Collection<CompromisedAssembly> components) {
		...
		List<DatamodelAttacker> dataList = new LinkedList<>(); 

		for (AssemblyContextDetail assemblyDetail : filteredComponents) {
			dataList.addAll(assemblyDetail.getAssemblyList().stream().distinct()
