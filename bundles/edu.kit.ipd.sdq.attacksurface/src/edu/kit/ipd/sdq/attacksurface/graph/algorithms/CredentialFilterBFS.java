...
public class CredentialFilterBFS extends BFSShortestPath<ArchitectureNode, AttackEdge> {
    ...
    public CredentialFilterBFS(final Graph<ArchitectureNode, AttackEdge> graph, final BlackboardWrapper modelStorage) {
        super(graph);
    ...
    @Override
    public SingleSourcePaths<ArchitectureNode, AttackEdge> getPaths(final ArchitectureNode source) {
         ...
         */
        final Map<ArchitectureNode, Pair<Double, AttackEdge>> distanceAndPredecessorMap = new HashMap<>();
        distanceAndPredecessorMap.put(source, Pair.of(0d, null));

        /*
         * Declaring queue
         */
        final Deque<ArchitectureNode> queue = new ArrayDeque<>();
        queue.add(source);

        final Set<UsageSpecification> gainedCredentials = new HashSet<>(this.attributeProvider(source.getEntity())
            .toList());
        /*
         ...
         */
        while (!queue.isEmpty()) {
            final var v = queue.poll();
            for (final AttackEdge e : this.graph.outgoingEdgesOf(v)) {
...
//                }
                final var u = Graphs.getOppositeVertex(this.graph, e, v);
                if (!distanceAndPredecessorMap.containsKey(u)) {
                    queue.add(u);
                    gainedCredentials.addAll(this.attributeProvider(u.getEntity())
                        .toList());
                    var newDist = 0D;
                    if (e.getCause() == null) {
                        newDist = distanceAndPredecessorMap.get(v)
                            .getFirst() + 10000.0;
                    } else {
                        newDist = distanceAndPredecessorMap.get(v)
                            .getFirst() + 1.0;
    ...
    private boolean credentialCheck(final AttackEdge edge, final Set<UsageSpecification> credentials) {
