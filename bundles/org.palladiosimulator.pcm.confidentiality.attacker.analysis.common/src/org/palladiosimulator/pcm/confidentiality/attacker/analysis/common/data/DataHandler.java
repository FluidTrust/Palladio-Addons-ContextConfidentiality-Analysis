...
public class DataHandler {
    ...
    public static Collection<DatamodelAttacker> getData(final AssemblyContext assemblyContext) {
        ...
        final var component = (BasicComponent) assemblyContext.getEncapsulatedComponent__AssemblyContext();

        final var dataList = component.getServiceEffectSpecifications__BasicComponent()
            .stream()
            .filter(ResourceDemandingSEFF.class::isInstance)
            .map(ResourceDemandingSEFF.class::cast)
            .flatMap(seff -> getData(seff, assemblyContext).stream())
            .collect(Collectors.toList());

        dataList.stream()
            .forEach(data -> data.setSource(assemblyContext));
        return dataList;
    ...
    public static List<DatamodelAttacker> getData(final ResourceContainer resource, final Allocation allocation) {
        ...
        return assemblyContexts.stream()
            .flatMap(e -> getData(e).stream())
            .collect(Collectors.toList());
    }

    public static Collection<DatamodelAttacker> getData(final ServiceSpecification serviceRestriction) {
        final var dataList = getData(serviceRestriction.getService(), serviceRestriction.getAssemblycontext());
        dataList.stream()
            .forEach(data -> data.setSource(serviceRestriction));
        return dataList;
    }
    ...
    private static Collection<DatamodelAttacker> getData(final ResourceDemandingSEFF seff,
        ...
        final var seffList = getExternalData(seff, context);
    ...
    private static Collection<DatamodelAttacker> getExternalData(final ResourceDemandingSEFF seff,
        ...
        return seff.getSteps_Behaviour()
            .stream()
            .filter(ExternalCallAction.class::isInstance)
            .map(ExternalCallAction.class::cast)
            .flatMap(action -> {

                final var context = PCMInstanceHelper.getHandlingAssemblyContexts(action, List.of(source));
                final var targetOperation = action.getCalledService_ExternalService();

                final var parameters = getParameters(List.of(targetOperation));

                final var data = createDataFromParameter(parameters, context);
    ...
    private static Stream<Parameter> getParameters(final Collection<OperationSignature> signatures) {
        return signatures.stream()
            .flatMap(signature -> signature.getParameters__OperationSignature()
                .stream());
